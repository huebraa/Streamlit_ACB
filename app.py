...
def mostrar_jugadores_por_posiciones_y_perfiles(df_filtrado, perfiles_seleccionados, max_jugadores, url_imagen):
    ...
    img_fondo = mpimg.imread('pista-acb.jpeg')

    # Dimensiones específicas de la imagen
    img_width, img_height = 535, 488

    # Crear figura y ejes con tamaño ajustado para la imagen de fondo
    fig, ax = plt.subplots(figsize=(img_width / 100, img_height / 100))  # Ajuste el tamaño a la imagen
    ax.imshow(img_fondo)  # Mostrar la imagen de fondo
    ax.axis('off')  # Quitar ejes

    # Tamaño de fuente ajustable según la resolución de la imagen
    font_size = 18  # Puedes aumentar o reducir según la imagen
    azul_color = '#1f77b4'

    # Coordenadas ajustadas para cada posición (basado en el tamaño de la imagen)
    posiciones_coordenadas = {
        "Base (PG)": (0.1, 0.85),  # Estas coordenadas son relativas a la imagen
        "Escolta (SG)": (0.2, 0.7),
        "Alero (SF)": (0.7, 0.65),
    ...
    for posicion, (x, y) in posiciones_coordenadas.items():
        ...
        if perfil_seleccionado and perfil_seleccionado != "Selecciona un perfil":
            ...
            ax.text(x * img_width, y * img_height, f"{posicion} - {perfil_seleccionado}", 
            ...
            if not jugadores_posicion.empty:
                ...
                ax.text(x * img_width, (y - 0.05) * img_height, lista_jugadores, 
                        ha="center", va="top", fontsize=font_size, color=azul_color)
            else:
                ax.text(x * img_width, (y - 0.05) * img_height, "Sin jugadores destacados", 
                        ha="center", va="top", fontsize=font_size, color="gray")

    # Título general centrado arriba
    ax.text(0.5 * img_width, 0.95 * img_height, "Jugadores destacados por posición y perfil seleccionado", 
